...
document.addEventListener("DOMContentLoaded", () => {
  drawChartBook("amour");
  drawChartBook("horreur");
  drawChartBook("inspirational");
  drawChartBook("auteur");
  drawChartBook("fiction");
  drawChartBook("poésie");
  drawChartBook("fantastique");
...
async function installPWA() {
  if (deferredPrompt) {
    ...
    deferredPrompt.userChoice.then(({ outcome }) => {
      if (outcome === "accepted") {
        console.log("Votre PWA a été installée");
      } else {
        console.log("L'utilisateur a choisi de ne pas installer votre PWA");
      }
      deferredPrompt = null;
    });
  }
